...
We now check for interactions

```{r}
reduced.int<-glm(formula=compare~waterfront*yr_built+condition_med*yr_built+view_med*yr_built+yr_built*yr_renovated,
                 family=binomial,data=train)
summary(reduced.int)
```

We check as well for any multicollinearity in our model by using the Variance Inflation Factors (VIF). When we run the tests, we find that all the remaining predictor variables in our model are well below 5, with all of them being between 1 and 1.3. This indicates that there is little correlation between all predictors present.
