...
selected_data$condition <- as.numeric(as.factor(selected_data$condition))
selected_data$grade <- as.numeric(as.factor(selected_data$grade))
selected_data$floors <- as.numeric(as.factor(selected_data$floors))
correlation_matrix <- selected_data %>% select(price, sqft_living, sqft_basement, condition, grade, floors, sqft_above) %>% cor()
...
$\begin{aligned} & \text { price }=β0+β1(\text { bedrooms })+β2(\text { bathroom })+β3(\text {sqft_living })+β4(\text {floors })+β5(\text {condition })+β6(\text {grade })+β7(\text { sqft_above})β8(\text { sqft_basement })\end{aligned}$

```{r echo=FALSE}
# Transformations and initial model fitting
kc_house_data$log_price <- log(kc_house_data$price)
initial_model <- lm(log_price ~ bedrooms + bathrooms + sqft_living + floors + condition + grade + sqft_above + sqft_basement, data = kc_house_data)
...
refined_model <- lm(refined_formula, data = kc_house_data)
```
...
$\begin{aligned} & \text { price }=β0+β1(\text { bedrooms })+β2(\text { bathroom })+β3(\text {sqft_living })+β4(\text {condition })+β5(\text {grade })+β6(\text { sqft_above})+β7(\text { sqft_basement })\end{aligned}$
...
kc_house_data <- kc_house_data[!high_influence, ]
...
final_model <- update(refined_model, data = kc_house_data)
summary(final_model)
```
## Split Data into Training and Testing Sets

To evaluate the model's generalizability, the data is split into training and testing sets. 70% of the data is used for training, while 30% is set aside for testing. This ensures that the model is evaluated on unseen data, providing an estimate of how well it might perform in real-world scenarios.


```{r echo=FALSE}
# Split data into training and testing sets
set.seed(6021)
training_indices <- createDataPartition(kc_house_data$log_price, p = 0.5, list = FALSE)
train_data <- kc_house_data[training_indices, ]
test_data <- kc_house_data[-training_indices, ]
```
...
$\begin{aligned} & \text { price }=β0+β1(\text { sqft_living })+β2(\text { grade })+β3(\text {condition })\end{aligned}$


```{r echo=FALSE}
# Fit the final model on training data
final_model_fit <- lm(log_price ~ sqft_living + grade + condition, data = train_data)
...
```{r echo=FALSE}
# Evaluate model performance on test data
test_predictions <- predict(final_model_fit, newdata = test_data)
test_residuals <- test_data$log_price - test_predictions
